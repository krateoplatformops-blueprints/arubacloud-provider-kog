...
// TestFlattenObject tests the FlattenObject function
func TestFlattenObject(t *testing.T) {
	testCases := []struct {
		...
		{
			name: "successful flattening of metadata object",
			inputJSON: `{
				"metadata": {
					"name": {
						"somethingelse": "value"
		...
		{
			name: "prefix is not an object",
			inputJSON: `{
				"metadata": "a string value",
				"other1": "foo"
			}`,
			prefix: "metadata",
			expectErr:      true,
			expectedErrMsg: "field 'metadata' is not a JSON object",
		},
		{
			name: "empty prefix",
			inputJSON: `{
			...
			prefix:         "",
			expectErr:      true,
			expectedErrMsg: "prefix cannot be empty",
		},
		{
			...
			prefix:         "metadata",
			expectErr:      true,
			expectedErrMsg: "failed to unmarshal JSON", // will check for prefix
		},
		{
			name: "nested field overwrites root field",
			inputJSON: `{
				"metadata": {
					"name": "nested name"
				},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result, err := FlattenObject([]byte(tc.inputJSON), tc.prefix)
			...
			// Compare JSON objects to ignore formatting differences
			var expected, actual map[string]interface{}
			if err := json.Unmarshal([]byte(tc.expectedJSON), &expected); err != nil {
				t.Fatalf("failed to unmarshal expected JSON: %v", err)
			}
			if err := json.Unmarshal(result, &actual); err != nil {
				t.Fatalf("failed to unmarshal actual result JSON: %v", err)
			}

			if !reflect.DeepEqual(expected, actual) {
				t.Errorf("expected JSON:\n%s\nbut got:\n%s", tc.expectedJSON, string(result))
			}
		})
	}
}